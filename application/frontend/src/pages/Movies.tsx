...
const MoviesTable = (props: TypeMoviesTable) => {
  ...
  return (
    <div className="space-y-4">
      <div className="overdflow-auto border">
        <table className="border bg-white w-full border-collapse border-hidden">
          <thead>
            <tr>
              <th className="p-2 text-start border">TMDB_ID</th>
              <th className="p-2 text-start border">Name</th>
              <th className="p-2 text-start border">Status</th>
              <th className="p-2 text-start border">Actions</th>
            </tr>
          </thead>
          <tbody>
            {currentPosts
              ? currentPosts.map((movie, index) => {
                  return (
                    <tr key={index}>
                      <td className="p-2 text-start border">{movie.id}</td>
                      <td className="p-2 text-start border">
                        <span className="line-clamp-1">{movie.title}</span>
                      </td>
                      <td className="p-2 text-start border">
                        <p
                          className={`text-sm p-1 w-max rounded-full text-center ${
      ...
      <div className="flex space-x-8">
        <div className="flex items-center space-x-2">
          <div><p>Showing page 1 of 1</p></div>
...
const DownloadButton = (props: DownloadButtonProps) => {
  ...
  return (
    <button
      className="flex items-center justify-center space-x-2 p-2 w-48 text-sm bg-green-100 text-green-600 font-medium"
      onClick={() => {
        downloadSubtitle();
      }}
